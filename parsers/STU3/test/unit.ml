...
let unwrap_msg = function `Msg m -> m | _ -> ""
...
let error =
  Alcotest.testable pp_error (fun a b ->
      String.( = ) (unwrap_msg a) (unwrap_msg b))
...
let test_patient_parse () =
  ...
  let patient =
    match patient with Ok p -> p | Error e -> fail (unwrap_msg e)
  in
  Alcotest.(check int)
    "Should have elements" 52
    (List.length (Stu3.Structure.elements patient));
  ()
...
let test_account_parse () =
  ...
  let account =
    match account with Ok a -> a | Error e -> fail (unwrap_msg e)
  in
  Alcotest.(check int)
    "Should have elements" 32
    (List.length (Stu3.Structure.elements account));
  ()
