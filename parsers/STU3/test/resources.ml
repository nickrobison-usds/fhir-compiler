...
let datatypeTest =
  testable
    (fun f t -> Fmt.pf f "%s" (Lib.Datatype.t_to_string t))
    (fun l r ->
      String.equal (Lib.Datatype.t_to_string l) (Lib.Datatype.t_to_string r))
...
let simple =
  S.create "Account" "Simple"
    [ Lib.Element.create "Elem.first" 0 "1" [ "string" ] ]
...
let simple_field : type a. a Lib.Fhir.field -> unit =
 fun field ->
  ...
  | Field field -> (
      ...
      | Scalar s ->
          Alcotest.check datatypeTest "Should be a string"
            (Lib.Datatype.Simple Lib.Simple_datatype.String) s.scalar_type;
          Alcotest.(check bool) "Should not be required" false s.required
      | _ -> Alcotest.fail "Must be a scalar")

let arity =
  S.create "Account" "Arity"
    [ Lib.Element.create "Elem.first" 1 "*" [ "integer" ] ]
...
let arity_field : type a. a Lib.Fhir.field -> unit =
 fun field ->
  ...
  | Field field -> (
      match field.datatype with
      | Arity a ->
          Alcotest.check datatypeTest "Should be an integer"
            (Lib.Datatype.Simple Lib.Simple_datatype.Integer) a.ft2
...
let complex_test () =
  ...
  let c =
    List.find_exn f.fields ~f:(fun field ->
        match field with
        | Lib.Fhir.Field field ->
            Stdio.print_endline (Printf.sprintf "Have field: %s" field.id);
            String.equal field.id "Patient.contact")
  in
  Alcotest.(check bool)
    "Should be complex" true
    (match c with
    | Lib.Fhir.Field c -> (
        match c.datatype with Lib.Fhir.Complex _ -> true | _ -> false));
  (* Let's see how many types we have in our complex*)
  Alcotest.(check int)
    "Should have a nested type" 1
    (match c with
    | Field c -> (
