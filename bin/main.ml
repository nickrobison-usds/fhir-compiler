...
let debug fmt s = Log.debug (fun m -> m fmt s)
...
let parser_version_str = function STU3 -> "STU3" | R4 -> "R4"

(* Common options *)
type copts = { file : string; parser : parser_version; backend : string }

let pr_copts fmt copts =
  Fmt.pr fmt "file = %s\n parser = %s" copts.file
    (parser_version_str copts.parser)
...
let parser_t =
  ...
  Arg.(
    value & opt parser ~vopt:STU3 STU3
...
let copts_t = Term.(const copts $ const "" $ parser_t $ backend_t $ setup_log)
...
let term =
  ...
  in
  let info = Cmd.info "fhirc" ~version ~doc ~sdocs ~exits ~man in
  Cmd.v info parse_cmd

let cmds = [ Swift.Main.Swift_compiler.commands ]
let () = Caml.exit (Cmd.eval term)
