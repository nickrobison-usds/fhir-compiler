...
let parent_tests () =
  ...
  Alcotest.(check bool)
    "Should be parent" true
    (Path.is_parent c1 (Path.from_string "Patient.Contact.Address"));
  Alcotest.(check bool)
    "Should not be parent" false
    (Path.is_parent
       (Path.from_string "Patient.Name")
       (Path.from_string "Patient.Contact.Address"));
  Alcotest.(check bool)
    "Should not be parent" false
    (Path.is_parent c1 (Path.from_string "Patient"))
