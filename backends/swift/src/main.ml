...
module Swift_compiler = struct
  ...
  type t = { output_dir : Fpath.t }

  let name = "Swift"
  let name_sanitize = Re.Str.regexp "/"
  let create config = { output_dir = config }
  let surround p1 p2 pp_v fmt v = Fmt.pf fmt "%a%a%a" p1 () pp_v v p2 ()

  let emit_to_file pth name emitter res =
    let name' = Re.Str.global_replace name_sanitize "+" name in
    let path =
      Fpath.add_seg pth (Fmt.str "%s.swift" name') |> Fpath.to_string
    in
...
let () = Lib.Backend.register (module Swift_compiler) Swift_compiler.name
