...
module Uri = struct
  ...
  let yojson_of_t t = `String (Uri.to_string t)
...
let emit_complex t = Fhir.Complex { name = replace_leading t.id; fields = [] }

let emit_single t code =
  match is_primitive_or_class code with
  | true -> (
      Log.debug (fun f -> f "Emitting primitive");
      match t.max with "1" -> emit_scalar t code | _ -> emit_arity t code)
  | false ->
...
let to_field t =
  ...
  | Some dt ->
      Some
        (Fhir.Field { id = t.id; path = Path.from_string t.path; datatype = dt })

let path t = Path.from_string t.path
