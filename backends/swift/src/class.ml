...
let src =
  Logs.Src.create "fhirc.backend.swift.class" ~doc:"Swift backend class emitter"
 ...
 fun t _path field ->
  ...
  | Field field -> (
      ...
      | Scalar f ->
          let value =
            Swift_field.create
              (Lib.Path.trailing field.path)
              false f.scalar_type f.required
          in
          if f.required then
            {
              t with
          ...
          else { t with fields = value :: t.fields }
      | Union u ->
          ...
          let value =
            Swift_field.create u.l2 false (Lib.Datatype.Domain sum_name) false
          in
          {
            t with
            fields = value :: t.fields;
            nested_enums = s :: t.nested_enums;
          }
      | Arity f ->
          let required = if f.min >= 1 then true else false in
          let value = Swift_field.create f.l3 true f.ft2 required in
          if required then
            {
              t with
              constructor = value :: t.constructor;
      ...
      | Complex c ->
          ...
          let value =
            Swift_field.create c.name false (Lib.Datatype.Domain nested_name)
              false
          in
          {
            t with
            fields = value :: t.fields;
            nested_classes =
              create (Lib.Structure.make nested_name c.fields)
              :: t.nested_classes;
          })
...
let emit_constructor fmt t =
  let args = Fmt.list ~sep:Fmt.comma Swift_field.emit_name_value_pair in
  let body_args = Fmt.list ~sep:Formatters.nline emit_constructor_body in
  Fmt.pf fmt "init %a{\n%a\n}" (Fmt.parens args) t.constructor body_args
    t.constructor

let emit_open fmt t = Fmt.pf fmt "%s" (if t.is_open then "open" else "")

let emit_class_body fmt t =
  let fields = Fmt.list ~sep:Fmt.cut Swift_field.emit in
...
let emit_class t =
  ...
  let classes =
    List.fold lst ~init ~f:(fun acc c ->
        let class_str =
          Fmt.str "%a class %s %a" emit_open c c.name
            (Fmt.braces emit_class_body)
            c
        in
        Fmt.str "%s\n\n%s" acc class_str)
  in
...
let emit oc t = Stdio.Out_channel.output_string oc (emit_class t)
