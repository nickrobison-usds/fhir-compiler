...
let src =
  Logs.Src.create "fhirc.parser.stu3.downloader" ~doc:"STU3 Resource Downloader"

module Log = (val Logs.src_log src : Logs.LOG)

let debug fmt s = Log.debug (fun f -> f fmt s)
let dir f = Fpath.(v f)
...
let body_writer body =
  ...
  Stdio.Out_channel.with_file (Fpath.to_string pth) ~f:(fun oc ->
...
let download uri =
  ...
  Cohttp_lwt.Body.to_string body >|= body_writer
...
let unzip pth pusher =
  ...
  let entries =
    List.filter
      ~f:(fun (e : Zip.entry) -> List.length (Re.matches reg e.filename) > 0)
      entries
  in
