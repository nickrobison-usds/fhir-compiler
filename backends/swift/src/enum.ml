...
let src =
  Logs.Src.create "fhirc.backends.swift.enum" ~doc:"Swift backend enum emitter"

module Log = (val Logs.src_log src : Logs.LOG)

type t = { name : string; cases : string list }

let from_code (c : Lib.Codesystem.code) = c.code

let create (cs : 'a Lib.Codesystem.t) =
  { name = cs.name; cases = List.map cs.concept ~f:from_code }
...
let pp fmt t = Fmt.pf fmt "%a" pp_enum_surround t
