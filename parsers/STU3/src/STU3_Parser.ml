...
module P = struct
  let name = "STU3"
  let to_resource structure = Structure.to_resource structure
  ...
  let t_of_yojson json = to_fhir json
  ...
  let parse () =
    ...
    let _ =
      ...
      Lwt.return_unit
    in
    stream |> Lwt_stream.map Yojson.Safe.from_string |> Lwt_stream.map to_fhir
end

let () = Lib.Parser.register (module P) P.name
